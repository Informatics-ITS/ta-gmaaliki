...
def split_tracklets(tmp_trklets, eps=None, max_k=None, min_samples=None, len_thres=None):
    ...
    for tid in tqdm(sorted(list(tmp_trklets.keys())), total=len(tmp_trklets), desc="Splitting tracklets"):
        ...
        if len(trklet.times) < len_thres:  # NOTE: Set tracklet length threshold to filter out short ones
            ...
            obj_class = np.array(trklet.obj_class)
            ...
            if not id_switch_detected:
                ...
                for label in unique_labels:
                    ...
                    tmp_class = obj_class[clusters == label]
                    assert new_id not in tmp_trklets
                    
                    tracklets[new_id] = Tracklet(new_id, tmp_frames.tolist(), tmp_scores.tolist(), tmp_bboxes.tolist(), feats=tmp_embs.tolist(), obj_class=tmp_class.tolist())
                    new_id += 1
...
def save_results(sct_output_path, tracklets):
    ...
    for i, tid in enumerate(sorted(tracklets.keys())): # add each track to results
        ...
        for instance_idx, frame_id in enumerate(track.times):
            ...
            obj_class = track.obj_class[0]
            
            results.append(
                [int(frame_id), tid, bbox[0], bbox[1], bbox[2], bbox[3], 1, int(obj_class), -1, -1]
            )
...
def refine_tracklets(
    track_src: str, # Path to the .pkl file.
    output_dir: str, # Path for saving the end results

    use_connect: bool = False, # Include this flag to use the connecting component.
    use_split: bool = False, # Include this flag to use the split component.
    min_len: int = 100, # Minimum length for a tracklet required for splitting (default is 100).
    eps: float = 0.6, # Maximum distance between two samples for DBSCAN clustering (default is 0.6).
    min_samples: int = 10, # Minimum number of samples in a neighborhood for a point to be considered a core point (default is 10).
    max_k: int = 3, # Maximum number of clusters/subtracklets to be output by the splitting component (default is 3).
    spatial_factor: float = 1.0, # Factor to adjust spatial constraints (default is 1.0).
    merge_dist_thres: float = 0.4, # Minimum cosine distance between two tracklets for merging (default is 0.4).
):
    # Determine the process based on the flags
    if use_split and use_connect:
    ...
    seq = os.path.basename(track_src)
    seq_name = seq.split('.')[0]
    with open(track_src, 'rb') as pkl_f:
    ...
    if use_split:
        ...
        splitTracklets = split_tracklets(tmp_trklets, eps=eps, max_k=max_k, min_samples=min_samples, len_thres=min_len)
    else:
    ...
    mergedTracklets = merge_tracklets(splitTracklets, seq2Dist, Dist, seq_name=seq_name, max_x_range=max_x_range, max_y_range=max_y_range, merge_dist_thres=merge_dist_thres)
    ...
    sct_name = f'{output_dir}/mot_results_gta_{process}_eps{eps}_minSamples{min_samples}_K{max_k}_mergeDist{merge_dist_thres}_spatial{spatial_factor}'
    os.makedirs(sct_name, exist_ok=True)
    new_sct_output_path = os.path.join(sct_name, '{}.txt'.format(seq_name))
    save_results(new_sct_output_path, mergedTracklets)
    return new_sct_output_path