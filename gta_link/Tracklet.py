class Tracklet:
    def __init__(self, track_id=None, frames=None, scores=None, bboxes=None, feats=None, obj_class=None):
        '''
        ...
        Args:
            ...
            obj_class (list or int, optional): Object class for the track. Defaults to None.
        '''
        ...
        self.obj_class = obj_class if isinstance(obj_class, list) else [obj_class] if obj_class is not None else []
        self.features = feats if feats is not None else []

    def append_det(self, frame, score, bbox, obj_class):
        '''
        ...
        self.obj_class.append(obj_class)
    ...
    def extract(self, start, end):
        ...
        ''', self.obj_class[start:end + 1]
        subtrack = Tracklet(self.track_id, self.times[start:end + 1], self.scores[start:end + 1], self.bboxes[start:end + 1], self.features[start:end + 1] if self.features else None, self.obj_class[start:end + 1])
        return subtrack